    ...
    foreach($datas as $data)
    {
        ...
        geocoder.geocode( { 'address': '". $data->postcode ."'}, function(results, status) {
          if (status == google.maps.GeocoderStatus.OK) {
            ...
            content: '<a href=\"".$url."\" target=\"_blank\">".$data->org_name."</a> already <a href=\"".get_permalink($data->post_id)."\">".get_the_title($data->post_id)."</a>'
        });
...
function reccomendations_page()
{
	...
	foreach ($posts as $post)
	{
		...
		echo "<p><a href=\"".get_permalink($post->ID)."\">Read the recommendation...</a></p>";
		echo "<div class='post-meta'>";
		$categories = get_the_category($post->ID);
		echo "<p>Line of enquiry: ";
...
add_shortcode('reccomendations_page', 'reccomendations_page');
...
//Public facing hooks
add_action('comment_form_before', 'create_wadt_post_form');
add_action('wp_loaded', 'submit_wadt');
add_action('wp_head', 'add_gmaps_js');
add_action('comment_form_before', 'show_wadt');
